...
//  server
//
//  Created by Chris Pauley on 10/10/16.
//  Copyright © 2016 Chris Pauley. All rights reserved.
//
//  Extended from https://github.com/civetweb/civetweb/blob/master/src/CivetServer.cpp
...
int CivetServer::requestHandler(struct mg_connection *conn, void *cbdata) {
    ...
    std::cout << request_info->remote_addr << " " << request_info->request_method << " " << request_info->request_uri << std::endl;
                      ...
                      size_t occurrence)
{
    ...
    if (data[0] == '{' || data[0] =='['){
        ...
        for (p = data; p + name_len < e; p++) {
            if ((p == data + 2 || p[-2] == ',') && p[name_len] == '"'
                && mg_strncasecmp(name, p, name_len) == 0 && 0 == occurrence--) {
                
                // Point p to variable value (after ": )
                p += name_len + 2;
